...
int main()
{
    struct ListNode node2 = {.val = 4, .next = NULL}, node4 = {.val = 4, .next = NULL}, node1 = {.val = 2, .next = &node2}, node3 = {.val = 3, .next = &node4}, list1 = {.val = 1, .next = &node1}, list2 = {.val = 1, .next = &node3}, *res = mergeTwoLists(&list1, &list2);
    while (res)
    {
        printf("%d ", res->val);
        res = res->next;
